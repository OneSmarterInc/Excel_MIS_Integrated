...
class BookImage(models.Model):
    """A picture lifted out of the uploaded book, kept beside the section it came from.

    The text of a chapter was always extracted; the figures were not, and a screenshot of
    a worksheet carries more than the paragraph describing it. Each picture remembers the
    text that surrounded it in the file, which is both how it finds its chapter or module
    and what the model is given when it writes the caption.
    """

    book = models.ForeignKey(Book, on_delete=models.CASCADE, related_name="images")
    chapter = models.ForeignKey(
        Chapter, null=True, blank=True, on_delete=models.CASCADE, related_name="images"
    )
    module = models.ForeignKey(
        Module, null=True, blank=True, on_delete=models.CASCADE, related_name="images"
    )
    ...
    # The words that sat around this picture in the original file.
    context_text = models.TextField(blank=True)
    caption = models.TextField(blank=True)
    # Set once the model has written the caption, so it is not asked twice.
    ...
    def __str__(self):
        return f"Figure on page {self.page} of {self.book.title}"
...
class Notification(models.Model):
    class Kind(models.TextChoices):
        ...
        APPROVAL_RESULT = "APPROVAL_RESULT", "Approval result"
        PUBLISHED = "PUBLISHED", "Material published"
        EVALUATION = "EVALUATION", "Evaluation given"
    ...
    link_path = models.CharField(max_length=250, blank=True)
...
def notify(recipient, title: str, message: str, kind: str = Notification.Kind.SYSTEM,
           sender=None, link_path: str = "") -> Notification:
    """Helper to dispatch in-app notification to a student, faculty, or admin."""
