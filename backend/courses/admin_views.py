...
def review_chapter(request, pk):
    ...
    notify(
        chapter.book.course.faculty,
        f"Chapter {chapter.number} {decision.replace('_', ' ')}",
        f"Admin {request.user.name} marked Chapter {chapter.number}: '{chapter.title}' as {chapter.status}." + (f" Comment: {comment}" if comment else ""),
        kind=Notification.Kind.APPROVAL_RESULT,
        sender=request.user,
    )
...
def approve_all(request):
    """Admin option to approve all modules and chapters in the review queue or for a specific book."""
    from .models import Notification, notify
    ...
    if book_id:
        ...
        queryset = queryset.filter(book__course_id=course_id)
    else:
        queryset = queryset.filter(status=Approval.UNDER_REVIEW)

    count = 0
    now = timezone.now()
    faculty_notified = set()
    for chapter in queryset:
        chapter.status = Approval.APPROVED
        chapter.reviewed_at = now
        chapter.review_comment = request.data.get("comment", "Approved by administrator.")
        chapter.save(update_fields=["status", "reviewed_at", "review_comment"])
        chapter.modules.update(status=Approval.APPROVED)
        ...
        faculty_notified.add(chapter.book.course.faculty)
        count += 1

    for fac in faculty_notified:
        notify(
            fac,
            "Chapters approved by admin",
            f"An administrator approved {count} chapter(s). You can now publish them whenever you are ready!",
            kind=Notification.Kind.APPROVAL_RESULT,
            sender=request.user,
        )
    ...
    return Response({
        ...
        "detail": f"Successfully approved {count} chapter(s) and their modules.",
    })
